...
class VKontakte {
    ...
    async defaultCallback (t, ctx) {
        ...
        bcContext.Message.sender = {
            id: ctx.payload.out === 1 ? this.BC.SELF_SEND : ctx.senderId,
    ...
    async reply (ctx, Parcel) {
        ...
        if (this.mode === this.MODES.GROUP && !this.BC.T.empty(this.BC.T.extract('keyboard.rows', Parcel))) {
    ...
    async fetchUserInfo (userId) {
        ...
        if (userId === this.BC.SELF_SEND || userId === 0 || userId === undefined) {
            ...
            // if (!this.BC.T.empty(userInfo) && !this.BC.T.empty(userInfo[0])) {
            //     userInfo = userInfo[0];
            // }
        }
        result = {
    ...
    async launch(middleware, ...middlewares) {
        ...
        switch (this.mode) {
            case this.MODES.GROUP:
                await this.Transport.updates.start().catch(console.error);
        ...
        this.Transport.api.users.get({fields: ['screen_name']}).then(user => this.user = user);
