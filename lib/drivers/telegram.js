...
class Telegram {
    ...
    reply (ctx, Parcel) {
        console.log(Parcel);
        if (ctx.reply) {
            return ctx.reply(Parcel.message, Parcel.keyboard).then(message => this.defaultCallback(this, {update: {message: message}}));
        }
    }

    send (Parcel) {
        console.log('TG SEND MESSAGE. IN DATA ', Parcel);
        return this.Telegram.sendMessage(Parcel.peerId, Parcel.message).then(message => this.defaultCallback(this, {update: {message: message}}));
    }

    launch (middleware, ...middlewares) {
        this.transport.launch(middleware, ...middlewares).then(() => {
