...
class Telegram {
    constructor (BC, params = {}) {
        ...
        let sessionDefaultHandler = this.BC.T.empty(this.BC.dbParams) ? LocalSession : MySQLSession;
        let sessionDefaultParams = this.BC.T.empty(this.BC.dbParams) ? {database: 'telegram_db.json'} : {
            host: this.BC.dbParams.host,
            user: this.BC.dbParams.username,
            password: this.BC.dbParams.password,
            database: this.BC.dbParams.database,
        };
