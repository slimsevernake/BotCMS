...
class Telegram {
    ...
    defaultCallback (t, ctx, own = false) {
    ...
    reply (ctx, Parcel) {
        console.log(Parcel);
        return ctx.reply(Parcel.message, Parcel.keyboard).then(message => this.defaultCallback(this, {update: {message: message}}, true));
    }

    send (Parcel) {
        console.log('TG SEND MESSAGE. IN DATA ', Parcel);
        return this.Telegram.sendMessage(Parcel.peerId, Parcel.message).then(message => this.defaultCallback(this, {update: {message: message}}, true));
    }
