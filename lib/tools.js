...
class Tools {
    ...
    async checkTrigger (ctx, triggerObj) {
        ...
        for (let triggerObject of tmpArray) {
            if (!this.empty(triggerObject)) {
                ...
                if (type === 'chatType') {
                    result = triggers.indexOf(message.chat.type) !== -1
                }

                if (type === 'regexp' && !this.empty(message.text)) {
                    triggerObject.value = this.MT.makeArray(triggerObject.value)
                ...
                if ((this.MT.empty(type) || ['text', 'method'].indexOf(type) > -1)) {
                    for (let trigger of triggers) {
                        ...
                        if (typeof method === 'function') {
                            ...
                            let translatedTrigger = await ctx.lexicon(trigger);
        ...
        if (this.MT.empty(result)) {
            ...
            if (!this.MT.empty(sw)) {
                ...
                if (this.MT.empty(ctx) && sw.hasOwnProperty(swKey)) {
                    ...
                    let lexiconSwKey = await ctx.lexicon(swKey);
                    if (sw.hasOwnProperty(lexiconSwKey)) {
                        actions = sw[lexiconSwKey];
                    } else {
                        for (let key in sw) {
                            if (sw.hasOwnProperty(key)) {
                                if (ctx.msg === key || ctx.msg === await ctx.lexicon(key)) {
                                    actions = sw[key];
                                    break;
                                }
                            }
            ...
            switch (validator) {
                ...
                case 'values':
                    ...
                    for (let value of validatorParams) {
                        if (ctx.msg === await ctx.lexicon(value)) {
                            result = true;
                            break;
                        }
                    }
                ...
                case 'answer':
                    result = ctx.getAnswer(validatorParams.thread, validatorParams.key)
                    break
    ...
    async execMessage (message, ctx, params = {}) {
        let method
        if (typeof message === 'object') {
            method = this.MT.extract(message.method ? message.method : message.text)
            params = this.MT.merge(message.params || {}, params)
        } else {
            method = this.MT.extract(message)
        }
        let result = typeof message === 'object' ? message.text : message
