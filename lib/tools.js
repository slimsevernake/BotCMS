class Tools {
    ...
    async checkTrigger (ctx, triggerObject) {
            ...
            if (!this.empty(triggerObject) && triggerObject.hasOwnProperty('type')) {
                ...
                if (triggerObject.type === 'text' && !this.empty(message.text)) {
                    ...
                    if (Array.isArray(trigger)) {
                        ...
                        result = trigger === message.text || translatedTrigger === message.text;
    ...
    extractAnswerThread (step) {
        if (!this.BC.MT.empty(step.store_thread)) {
            return step.store_thread;
        } else {
    ...
    execMessage (message, ctx) {
        let method = this.extract(message);
