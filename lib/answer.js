...
class Answer {
    ...
    async build () {
        ...
        if (this.MT.empty(this.success)) {
            actions = this.T.extract('validate.failure', this.step);
        } else {
            let sw = this.MT.extract('validate.switch', this.step);
            if (!this.MT.empty(sw)) {
                for (let key in sw) {
                    if (sw.hasOwnProperty(key)) {
                        if (this.Context.msg === this.Context.lexicon(key)) {
                            actions = sw[key];
                            break;
                        }
                    }
            ...
            if (Object.keys(actions).length === 0) {
                actions = this.MT.extract('validate.success', this.step);
            }
        }
    ...
    async validate () {
        ...
        let validatorParams = this.T.extract('validate.params', this.step);
        if (this.MT.empty(validatorParams)) {
            validatorParams = this.T.extract('validate.validator-params', this.step);
        }
