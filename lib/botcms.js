...
class BotCMS {
    ...
    async doUpdate (current, ctx, opts = {}) {
        ...
        let result = null;
        let keyboard = await (new this.config.classes.Keyboard(ctx))
        ...
        if (!this.MT.empty(current.message)) {
            ...
            parcel.editMsgId = !current.replace ? 0 : (
              ctx.Message.author.id === this.SELF_SEND ? ctx.Message.id : (opts.formMsgId || 0)
            )
    ...
    isFormQuestion (step) {
        return typeof step.form === 'object' && step.form.question === true
    }
