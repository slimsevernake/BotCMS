...
class BotCMS {
    ...
    async handleUpdate (ctx) {
        ...
        if (step !== undefined) {
            // let actions = await this.T.processGotos(ctx, step.goto);
            ...
            await this.doMethods(answer.methods);
            await this.doHelp(ctx, answer.help, opts);
            nextStep = await this.findNextStep(answer, ctx);
        }
        ...
        // if (step !== undefined) {
        //
        //     let nextStep = await this.findNextStep(answer, ctx);
    ...
    async doUpdate (current, ctx, updateSession = true, opts = {}) {
        ...
        if (!this.MT.empty(goto)) {
            ...
            // let newCurrent = this.Scripts.extract(actions.goto);
            // // console.log(JSON.stringify(this.schema.scripts));
            ...
            // } else {
            //     console.error('PATH ' + actions.goto + ' NOT FOUND');
            // }
        }
        return result;
    }
