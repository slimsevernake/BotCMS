...
class BotCMS {
    ...
    async doUpdate (current, ctx, opts = {}) {
        ...
        if (!this.MT.empty(current.message)) {
            ...
            parcel.editMsgId = !current.replace ? 0 : (
              ctx.Message.author.id === this.SELF_SEND && !this.isFormQuestion(opts.fromStep) ? ctx.Message.id : (opts.formMsgId || 0)
            )
            result = await ctx.reply(parcel);
            // console.log(current, result)
            await this.formStore(ctx, current, result)
        }
    ...
    isFormQuestion (step) {
        return typeof step === 'object' && typeof step.form === 'object' && step.form.question === true
    }

    async formStore (ctx, step, ids = []) {
        // console.log('FORM STORE. STEP PATH', step.path, 'STEP FORM', step.form, 'MSG', ctx.msg, 'IDS', ids)
        let form = {}
        ...
        if (isFormOpen) {
            ...
            ctx.session.form = {
                ...
                formMsgId: ctx.Message.id,
                formQuestionIds: []
            }
        } else if (this.isFormQuestion(step)) {
            // console.log('FORM STORE. IF SECTION IS FORM QUESTION')
            if (ctx.session.form) {
                ctx.session.step = step
                ids = this.MT.makeArray(ids)
                for (let id of ids) {
                    if (ctx.session.form.formQuestionIds.indexOf(id)) {
            ...
            if (ctx.session.form) {
                ...
                if (form.formQuestionIds.length) {
                    await ctx.remove(form.formQuestionIds, form.formChatId)
                }
            }
        }
    ...
    storeCBData (cbData) {
        ...
        this.StorageManager.storeSet(key, data)
        return key
    }
}
