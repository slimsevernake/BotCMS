...
class BotCMS {
    ...
    async handleUpdate (ctx) {
        ...
        if (step !== undefined) {
            ...
            if (!this.T.empty(step.store)) {
                this.storeAnswer(answer, step, ctx, (!this.MT.empty(step['store-clean']) || !this.MT.empty(step['store_clean'])));
            }
    ...
    storeAnswer (answer, step, ctx, cleanOld = false) {
        const thread = this.T.extractAnswerThread(step);
        ctx.session.answers = ctx.session.answers || {};
        if (cleanOld || !(ctx.session.answers[thread] instanceof Object)) {
            ctx.session.answers[thread] = {};
        }
        let key = step.store !== true ? step.store : Object.keys(ctx.session.answers[thread]).length + 1;
        ctx.session.answers[thread][key] = {
