...
class BotCMS {
    ...
    async initDB() {
        ...
        } else if (!this.T.empty(this.config.db.type)) {
    ...
    async doUpdate (current, ctx, updateSession = true, opts = {}) {
        ...
        if (!this.T.empty(current.store_pre)) {
            this.storeAnswer(current, ctx, {}, true);
        }
        let result = null;
        const kbName = current.keyboard_name;
        if (kbName && this.keyboards[kbName] !== undefined) {
            current.keyboard = this.keyboards[kbName];
        }
        let keyboard = new this.config.classes.KeyBoard(ctx, current.keyboard).build();
    ...
    doAction(step, ctx = undefined) {
        if (!this.T.empty(step.action)) {
            ...
            switch (step.action.type) {
                ...
                case 'method':
                    let method = this.T.extract(step.action.name) || this.T.extract(step.action.value);
                    let params = this.T.extract(step.action.params, process, {});
                    if (method) {
                        method(ctx, params);
                    } else {
    ...
    storeAnswer (step, ctx, answer = {}, pre = false) {
        console.log('STORE ANSWER. ANSWERS ', ctx.session.answers);
        ...
        console.log('STORE ANSWER. ANSWERS THREAD ', ctx.session.answers[store.thread]);
        let key = store.key || Object.keys(ctx.session.answers[store.thread]).length + 1;
        ctx.session.answers[store.thread][key] = {
            message: step.message,
            answer: ('value' in store) ? store.value : this.MT.extract('Message.text', answer, ''),
        };
        console.log('STORE ANSWER. ANSWERS THREAD UPDATED ', ctx.session.answers[store.thread]);
    }
