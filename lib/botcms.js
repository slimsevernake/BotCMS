...
class BotCMS {
    ...
    doAction(step, ctx = undefined) {
        if (!this.MT.empty(step.action)) {
            ...
            switch (step.action.type) {
                case 'send':
                    let fromScope = this.MT.extract('action.options.from_scope', step);
                    let targets = this.MT.extract('action.options.target', step);
                    ...
                    if (!this.MT.empty(fromScope) && !this.MT.empty(ctx)) {
                        ...
                        if (!this.MT.empty(answers[fromScope])) {
                            message = this.MT.extract('action.options.message', step) + "\n\n";
                ...
                case 'method':
                    ...
                    let params = step.action.params ? this.MT.extract(step.action.params, process, {}) : {};
                    if (method) {
                        method(ctx, params);
                    } else {
    ...
    storeAnswer (step, ctx, answer = {}, pre = false) {
        ...
        store.thread = store.thread || this.MT.extractAnswerThread(step);
