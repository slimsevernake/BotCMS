...
class BotCMS {
    ...
    constructor (config = {}) {
        ...
        this.MiddlewareManager = new this.config.classes.MiddlewareManager(this);
        this.DB = null;
    ...
    async doUpdate (current, ctx, updateSession = true, opts = {}) {
        ...
        let result = null;
        let keyboard = (new this.config.classes.Keyboard(ctx, current.keyboard_name || current.keyboard)).build();
        ...
        if (!this.MT.empty(step.action)) {
            ...
            switch (type) {
                case 'send':
                    ...
                    let message = ctx ? ctx.lexicon(step.message) : this.Lexicons.process(step.message);

                    if (!this.MT.empty(fromScope) && !this.MT.empty(ctx)) {
                        let answers = ctx.session.answers;
                        if (!this.MT.empty(answers[fromScope])) {
                            message = ctx.lexicon(this.MT.extract('message', params, step.message)) + "\n\n";
                            for (let key in answers[fromScope]) {
                                if (answers[fromScope].hasOwnProperty(key)) {
                                    message = message + ctx.lexicon(answers[fromScope][key].message) + '>> ' + answers[fromScope][key].answer + "\n\n";
                                }
                            }
                        }
                    }
