...
class BotCMS {
    ...
    loadSchema (schema = {scripts: {}, config:{}, cron: {}}) {
        if (!this.T.empty(schema.keyboards)) {
            this.loadKeyboards(schema.keyboards);
        }
    ...
    initDB() {
        ...
        this.DB = createConnection(this.dbParams).then(connection => {
            this.DB = connection;
        }).catch(error => {
            this.failDB(error)
        });
    ...
    async handleUpdate (ctx) {
        ...
        // if (this.T.isCommand(ctx. message.text)) {
            let step = this.getStepByCommand(ctx);
            if (step !== undefined) {
            ...
            if (!this.T.empty(answer.goto)) {
                ...
                if (!this.T.empty(next)) {
                    if (this.T.isChildrenPath(answer.goto)) {
                        for (let key in next) {
                            if (next.hasOwnProperty(key) && this.T.checkTrigger(ctx, next[key].trigger)) {
                        ...
                        for (let name in targets) {
                            if (targets.hasOwnProperty(name) && !this.T.empty(this.bridges[name])) {
                                ...
                                for (let peer of peers) {
                                    ...
                                    let parcel = new this.classes.Parcel();
                                    peer = (peer === '__self__') ? ctx.Message.sender.id : peer;
                                    parcel.peerId = peer;
    ...
    getStepByCommand (ctx) {
        ...
        for (let i in this.commands) {
            ...
            if (step !== undefined && !this.T.empty(step.trigger) && this.T.checkTrigger(ctx, step.trigger)) {
