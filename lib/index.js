...
class BotCMS {
    ...
    async loadSchema (schema = {scripts: {}, config:{}, cron: {}}) {
        ...
        if (!this.T.empty(schema.middlewares)) {
            this.loadMiddlewares(schema.middlewares);
        }
    ...
    doUpdate (current, ctx, updateSession = true, opts = {}) {
        ...
        if (!this.T.empty(current.message)) {
            ...
            parcel.message = this.T.execMessage(current.message, ctx);
