    ...
    constructor (ctx, kbObject) {
        ...
        this.addBtnRow = (buttons) => {
            ...
            for (let button of buttons) {
                ...
                if (typeof button === 'string') {
                    button = this.BC.lexicon(button, {}, this._language, this.ctx)
                } else if (typeof button === 'object') {
                    if (!button.data) {
                        button.data = button.text
                    }
                    button.text = this.BC.lexicon(button.text, button.params || {}, this._language, this.ctx)
                }
        ...
        this.fromKBObject = async (kbObject = {}) => {
            if (kbObject.method !== undefined) {
                ...
                if (method) {
                    kbObject = await this.BC.MT.merge(method(this.ctx), kbObject);
                }
            }
            this.fromKBObjectSync(kbObject)
        }

        /**
         * Load all kb properties (except "method") from Object
         * @function
         * @param {kbObject} kbObject
         * @returns {Keyboard}
         */
                    ...
                    if (Object.prototype.hasOwnProperty.call(this.buttons, row)) {
                        ...
                        for (let i in buttonsRow) {
                            if (Object.prototype.hasOwnProperty.call(buttonsRow, i)) {
                                ...
                                buttonsRow[i].data = this.ctx.storeCallbackData(buttonsRow[i])
                            }
                        }
                    }
                }
